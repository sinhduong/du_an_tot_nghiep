                                    ...
                                    <div class="room-image" style="flex: 0 0 130px;">
                                        @if ($booking->rooms->isNotEmpty() && $booking->rooms->first()->roomType && $booking->rooms->first()->roomType->roomTypeImages->isNotEmpty())
                                            @php
                                                ...
                                                $imageUrl = $mainImage ? Storage::url($mainImage->image) : 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/249146036.jpg';
                                            @endphp
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        dots.forEach((dot, index) => {
            dot.addEventListener('click', function (e) {
                ...
                // Kiểm tra không gian bên trong card
                const spaceBelow = window.innerHeight - rect.bottom;
                const dropdownHeight = 80; // Ước lượng chiều cao của dropdown

                // Toggle dropdown
                const isOpen = dropdown.style.display === 'block';
                dropdowns.forEach(d => d.style.display = 'none'); // Đóng tất cả dropdown khác

                if (!isOpen) {
                    // Đặt vị trí ban đầu bên trong card
                    dropdown.style.top = '40px';
                    dropdown.style.right = '20px';
                    dropdown.style.position = 'absolute';
                    dropdown.style.display = 'block';

                    // Kiểm tra nếu bị cắt (không đủ không gian bên dưới)
                    if (spaceBelow < dropdownHeight && rect.top > dropdownHeight) {
                        // Tràn ra ngoài, đặt trên nút ba chấm
                        dropdown.style.top = (-dropdownHeight - 5) + 'px';
                    }

                    dot.style.color = '#007bff'; // Change color when open
                } else {
                    dropdown.style.display = 'none';
                    dot.style.color = '#555'; // Reset color when closed
                }
            });
        });
        ...
        document.querySelectorAll('.delete-booking').forEach(button => {
            button.addEventListener('click', function (e) {
                ...
                if (confirm('Bạn có chắc muốn xóa đơn đặt này?')) {
                    fetch(`/client/bookings/${bookingId}`, {
                        method: 'DELETE',
                        headers: {
